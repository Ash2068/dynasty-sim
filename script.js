...
window.onload = () => {
    ...
    if (archiveBtn && graveyard.length > 0) archiveBtn.style.display = 'block';
    showUI('setup');
    rollStats();
};
...
function postSocial(type) {
    ...
    updateLog(`${e.label} (${fameDelta >= 0 ? '+' : ''}${fameDelta} Fame)`);
    updateUI();
}
...
function renderFamily() {
    ...
    el.innerHTML = fam.map(f => `<div class="sub-box">${f.name} (${f.type}) — Relationship: ${f.rel}</div>`).join('')
...
function renderPets() {
    ...
    el.innerHTML = pets.map((pet, i) => `
        <div class="sub-box">
            ${pet.name} (${pet.species}, age ${pet.age}) — Bond: ${pet.relationship}
        ...
        </div>`).join('') || '<p style="color:#666;">No pets yet.</p>';
}
